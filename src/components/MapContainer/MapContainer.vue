	...
	export default {
		...
		methods: {
			...
			setMarker(){
				...
				this.map.plugin(['AMap.ElasticMarker'] , ()=>{
					this.map.remove(this.markers);
					this.markers = [];
					var churchList = this.churchList[this.church.type - 1];
					...
					churchList.forEach((item) => {
						let marker = new AMap.ElasticMarker({
							...
							extData: {id: 1,name: this.church.name,position: item.join(',')},
